...
int create_file(const char* filename, const uint8_t* content, uint32_t size) {
    ...
    // Find an empty spot in the file table
    int file_index = -1;
    for (int i = 0; i < MAX_FILES; i++) {
    ...
    file_table[file_index].is_occupied = 1;  // Mark the file as occupied

    save_file_table();  // Persist the changes
    return 0;  // Success
}
...
char* list_files() {
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        if (file_table[i].filename[0] != '\0') {
            pos += custom_snprintf(buffer + pos, sizeof(buffer) - pos, "File: %s, Size: %u bytes\n",
